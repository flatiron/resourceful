...
Couchdb.prototype.update = function (id, doc, callback) {
 if (this.cache.has(id)) {
   ...
   this.put(id, resourceful.mixin({}, r.toJSON(), doc), callback)
 } else {
   var that = this;
...
Couchdb.prototype.destroy = function (id, callback) {
  ...
  callback = function (err, res) {
    ...
    cb(null, that.cache.get(id) || { _id: res.id, _rev: res.rev });
  };
...
Couchdb.prototype.find = function (conditions, callback) {
  ...
  mapFunction+= "}()) { var t = doc._id; t = t.split('/').slice(1).join('/');  emit(t, doc); }}";
