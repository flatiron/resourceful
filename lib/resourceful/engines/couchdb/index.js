...
var Couchdb = module.exports = function Couchdb(config) {
  if (!(this instanceof Couchdb)) return new Couchdb(config);
...
Couchdb.prototype.update = function (id, doc, callback) {
 if (this.cache.has(id)) {
   ...
   this.put(id, common.mixin({}, r, doc), callback)
 } else {
   var that = this;
...
Couchdb.prototype.find = function (conditions, callback) {
  this.connection.temporaryView(common.render({
    map: function (doc) {
      var obj = $conditions;
      if (function () {
        for (var k in obj) {
